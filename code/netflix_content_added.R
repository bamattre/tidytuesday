...
data2 <- data %>%
  mutate(age = added_year - release_year)

library(ggridges)

data2 %>%
  filter(added_year > 2014,
         age <= 25) %>%
  ggplot(aes(x = age, y = factor(added_year))) +
    geom_density_ridges(
      jittered_points = TRUE, position = "raincloud",
      alpha = 0.8, fill = "#E50914"
    ) +
    theme_minimal() +
    facet_wrap(~ type) +
    labs(y = "Year Added", x = "Age of title when added (in years)",
         title = "Netflix: Age of Content When Added")
